...
- 不写按钮名称或参数值。
- 放在独立 `text` 代码块中。
- 如果是生成场景、背景、关键帧或风格参考图，使用 `Rich Visual Image Prompt` 的画面层次写法：主体、前景、中景、背景、主色调、光影、材质细节、风格转译和情绪。
- 如果是 `blend` 或 `inpaint`，仍保持局部任务短，但必须说明保留哪些高质量画面特征，例如主体轮廓、主色调、光源方向、材质细节和构图。
